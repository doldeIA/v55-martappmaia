...
import { ArrowLeftIcon, BuildingStorefrontIcon, TrashIcon, PlusCircleIcon } from './Icons';
...
export const GerenciarMarcasModal: React.FC<GerenciarMarcasModalProps> = ({ onClose, brands, onAddBrand, onRemoveBrand }) => {
            ...
            >
                <header className="relative flex items-center justify-center p-4 border-b border-white/10 flex-shrink-0">
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <h2 className="text-xl font-bold">Gerenciar Marcas</h2>
                    </div>
                </header>
                    ...
                    <ul className="space-y-2">
                        {brands.map((brand, index) => (
                            <li key={index} className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                                ...
                                <button onClick={() => onRemoveBrand(brand)} className="p-1 text-red-400 hover:text-red-300 hover:bg-white/10 rounded-full transition-colors">
                                    <TrashIcon className="w-5 h-5"/>
                                </button>
                            </li>
                        ))}
                         {brands.length === 0 && (
                            <li className="text-center text-sm text-white/50 py-8">Nenhuma marca cadastrada.</li>
                        )}
                    </ul>
                </main>
                ...
                <footer className="p-4 border-t border-white/10 flex-shrink-0">
                    <div className="flex items-center gap-2">
                        <input
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 focus:outline-none focus:ring-1 focus:ring-fuchsia-400"
                        />
                        <button 
                            onClick={handleAdd} 
